...
(function() {
  ...
  render_single_views = function(config, type, posts) {
    ...
    return async.each(posts(function(post, callback) {
      var compiler, locals, output, tpl;
      tpl = path.join(this.roots.root, config.template);
      locals = _.merge(this.roots.config.locals, {
        post: post
      });
      ...
      return callback(null, async.waterfall([
        function(callback) {
          return callback(null, compiler.renderFile(tpl, locals));
        }, function(err, res, callback) {
          console.log(res);
          return callback(null, this.util.write(output, res.result));
        }
      ], callback(null, true)));
    }));
  };
