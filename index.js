...
(function() {
  var API, RootsUtil, W, _, add_posts_to_locals, add_urls_to_posts, get_posts_and_routes, path, render_single_views, request;
  ...
  render_single_views = function(config, type, posts) {
    ...
    return W.map(posts, (function(_this) {
      return function(p, index) {
        var locals, output, tpl;
        tpl = path.join(_this.roots.root, config.template);
        locals = _this.roots.config.locals.wordpress[type][index];
        output = type + "/" + p.slug + ".html";
        return {
          output: output,
          locals: locals,
      ...
      return function(postArray) {
        var compiler;
        postArray.forEach(function(post) {
          if (post && post.tpl) {
            locals = _.merge(_this.roots.config.locals, { post: post.locals });
            compiler = _.find(_this.roots.config.compilers, function(c) {
              return _.contains(c.extensions, path.extname(post.tpl).substring(1));
            });
            compiler.renderFile(post.tpl, locals).then(function(res) {
              return _this.util.write(post.output, res.result).then(function() {
                return true;
              });
            }).then(function() {
              return post.output;
